...
- **`GET /paste/{uuid}/raw` je jediná route, která čte `PasteStore` přes `get` (read-only), ne `record_view`** — nesmí se počítat jako druhé "zobrazení" nad rámec toho, co už zaznamenala hlavní stránka (viz [ADR 0011](./adr/0011-octet-stream-preview-page.md)). Za `/raw` stojí dispatcher `render::render_raw_content`, který podle `MimeKind` zavolá `render_octet_stream_raw` (download/`<img>` zdroj), `render_html_raw` (`<iframe>` zdroj, viz [ADR 0013](./adr/0013-html-iframe-wrapper.md)), nebo vrátí `AppError::NotFound` pro `PlainText`/`Markdown` — ty žádnou "raw" formu odlišnou od své hlavní stránky nemají.
- **`render_paste_page` je čistý dispatch, jeden řádek na `MimeKind`** — `render_plain_text_page`, `render_html_page`, `render_markdown_page`, `render_octet_stream_page` jsou navzájem rovnocenné pomocné funkce (viz [ADR 0013](./adr/0013-html-iframe-wrapper.md)), žádná z nich není renderovaná inline v samotném matchi. Oddělené od nich jsou `render_octet_stream_raw`/`render_html_raw` (privátní, volané jen přes `render_raw_content`), protože je potřebuje jen `/raw`, ne hlavní stránka.
- **`render::html_escape` má dvě volající místa se stejným důvodem** — `render_octet_stream_page` (pro `file_name`) i `render_plain_text_page` (pro obsah pastu) escapují uživatelem kontrolovaná data těsně před vložením do HTML šablony (viz [ADR 0011](./adr/0011-octet-stream-preview-page.md), [ADR 0012](./adr/0012-plain-text-html-page.md)). `render_html_page` do šablony vkládá jen server-generovanou `/raw` URL, ne uživatelův obsah, takže escapování nepotřebuje. `MimeKind::Html` je jediný typ, kde se samotný obsah pastu záměrně needituje/nesanitizuje (NFR-7/NFR-9) — u něj je "beze změny" explicitní požadavek, jen se teď týká `GET /paste/{uuid}/raw`, ne přímo hlavní stránky (viz ADR 0013).
...
- Zámek `PasteStore` se získává vždy jen na dobu nutnou k operaci nad `Vec<Paste>` (`insert`, `record_view`, `get`) — v `routes::get_paste` handler zavolá `paste_store.record_view(uuid)?`, čímž zapíše `hits`/`last_seen_tick`, a teprve poté (se zámkem stále drženým, protože `paste` je `&Paste` půjčené z `MutexGuard`) volá `render::render_paste_page`. Renderovací funkce samy o sobě zámek nedrží ani neznají — ale protože `paste` je reference odvozená z `MutexGuard`, zámek zůstává držený i po dobu renderování markdownu/syntaxe/mermaidu, dokud handler nedoběhne. Stejný vzor platí i pro `routes::get_paste_raw` (`paste_store.get(uuid)?` → `render::render_octet_stream_raw`), jen s read-only zámkem místo mutable. To je zdokumentovaný kompromis, viz Otevřené otázky níže.
...
| [0013](./adr/0013-html-iframe-wrapper.md) | `Html` dostal iframe wrapper (`templates/html_page.html`, pevná výška), `/raw` zobecněný pro `Html`, tlačítko zpět ve všech čtyřech šablonách, `render_paste_page` jako čistý dispatch | Accepted |
