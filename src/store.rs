...
impl PasteStore {
    ...
    pub fn new (max_pastes: usize, max_paste_size: usize, max_file_size: usize) -> Self {
        PasteStore { pastes: Vec::new(), max_pastes, max_paste_size, max_file_size, current_tick: 0 }
    }

    fn max_content_size (&self, mimetype: &MimeKind) -> usize {
        match mimetype {
            MimeKind::OctetStream => self.max_file_size,
            _ => self.max_paste_size,
        }
    }
    ...
    pub fn insert (&mut self, content: Vec<u8>, mimetype: MimeKind, file_name: Option<String>) -> Result<Uuid, AppError> {
        let max_content_size = self.max_content_size(&mimetype);
        let paste = Paste::new(content, mimetype, self.get_next_tick(), max_content_size, file_name)?;
        let id = paste.id;
...
use super::*;
    fn create_empty_store (max_pastes: usize) -> PasteStore {
        PasteStore::new(max_pastes, 1_048_576, 20_971_520)
    }
    ...
    fn max_paste_1 () {
        ...
        paste_store.insert(format!("Content {}", 0).into_bytes(), MimeKind::PlainText, None).expect("Insert failed");
        paste_store.insert(format!("Content {}", 1).into_bytes(), MimeKind::PlainText, None).expect("Insert failed");
